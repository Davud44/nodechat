...
app.post("/register_save", (req, res) => {
  ...
  var con_req_check =
    "select count(*) as say from users where email = '" + email + "' ";
...
app.post("/login_check", async function(req, res) {
  ...
  var check = 0;
  var con_req =
    "select * , count(*) as say from users where email = '" +
    email +
    "' and password = '" +
    password +
    "' ";
...
io.on("connection", socket => {
  ...
  socket.on("chat message", msg => {
    ...
    var con_req =
      "insert into chat (fromm , too , message , time ) values ( '" +
      me +
      "' , '" +
      to +
      "' , '" +
